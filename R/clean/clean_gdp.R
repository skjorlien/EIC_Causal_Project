...
# Date: 5/8/2022
# Description: take the BEA scraped data for gdp by state,
...
df <- read_csv(here("data/raw/BEA/real_gdp.csv")) %>% 
  ...
  filter(year >= 2007) %>% 
  drop_na() %>% 
  select(state_name, state_code, year, m_gdp) %>% 
  rename("state" = state_name,
         "sfip" = state_code) %>%  
  write_csv(here("data/clean/state_gdp.csv"))
